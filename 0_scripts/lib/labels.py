...
def party_tags(model, country, grayscale=False):
    ...
    if country=='USA':
        democrats = [d for d in model.docvecs.index_to_key if d.startswith('D_')]
        republicans = [d for d in model.docvecs.index_to_key if d.startswith('R_')]
        ...
        labour = [d for d in model.docvecs.index_to_key if 'Lab' in d]
        liberals = [d for d in model.docvecs.index_to_key if 'Lib' in d]
        conservatives = [d for d in model.docvecs.index_to_key if 'Con' in d]
